...
int debug = 1;									// 1 - print debug messages, 0 - no messages
int MAX_LEN = 100;								// The maximum file name length
char FNAME_TEMPLATE[] = "./temp/temp_%u_%u.bin";// File name template
...
int oocmerge(unsigned int N, char* output_file){
	/* 
	Merges <N> in ./temp containing floating point numbers a writes the sorted numbers into one output_file
	...
	*/ 
	...
	// Define:
	unsigned int round = 1, num_files = N;				//  - starting round number
	...
	while (num_files > 1){
		...
		if( access(s1, R_OK) != 0) { 	// File fp1 DNE
			...
			if( access(s2, R_OK) != 0){ 	// File fp2 DNE
				...
				//decrement by 1 as this processes 2 files into 1 file
				num_files--;
			}
		}
	...
	if (N > 1) {
		...
		sprintf(s_last, FNAME_TEMPLATE, round, 1);
	}
